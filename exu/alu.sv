...
module alu (
    ...
    output logic [BP_ADDR_SIZE-1:0] instr_tag_exu,
    output logic exu_br_dir,                      // 1-> Taken, 0-> Not taken
//    output logic [XLEN-1:0] exu_pc_in,
  ...
  assign {pc_cout, pc} = ({XLEN+1{(alu_ctrl.jal & alu_ctrl.pc) | (alu_ctrl.condbr & brn_taken)}} & (alu_ctrl.imm + alu_ctrl.instr_tag[XLEN-1:0])) |
                         ({XLEN+1{alu_ctrl.condbr & ~brn_taken}}) & (alu_ctrl.instr_tag[XLEN-1:0] + 32'h4) | 
  ...
  assign flush = pc_vld ^ alu_ctrl.branch_taken;
  
  assign branch = (alu_ctrl.jal | alu_ctrl.condbr) & alu_ctrl.legal & ~alu_ctrl.nop & alu_ctrl.alu;
  ...
  dff_rst_vector #(
      ...
      .clk  (clk),
      .rst_n(rst_n),
      .reset_val({$bits({alu_wb_data, alu_wb_rd_addr, alu_wb_rd_wr_en}){1'b0}}),
  ...
  dff_rst_vector #(.WIDTH(XLEN + 1 + BP_ADDR_SIZE + 2)) pc_ff (
    .clk(clk),
    .rst_n(rst_n),
    .reset_val({XLEN+1+ BP_ADDR_SIZE + 2{1'b0}}),
    .din({pc, flush, alu_ctrl.instr_tag[BP_ADDR_SIZE+1:2], pc_vld, branch}),
    .dout({pc_out, pc_load, instr_tag_exu, exu_br_dir, exu_bp_strobe})
  );
  
  assign bp_true_out = alu_ctrl.bp_true;
endmodule
